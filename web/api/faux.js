...
await (async () => {
  ...
  let t1 = await Memoize.Type(Task).create(
    new Task(1, TaskState.Ready, "Task 1")
  );
  let t2 = await Memoize.Type(Task).create(
    new Task(2, TaskState.Active, "Task 2")
  );
  let t3 = await Memoize.Type(Task).create(
    new Task(3, TaskState.Done, "Task 3")
  );

  let u1 = await Memoize.Type(User).create(
    new User(1, "king@makeitall.co.uk", UserRank.ProjectManager, "King Firat")
  );
  u1.tasks.add(t2);
  let u2 = await Memoize.Type(User).create(
