...
2. The system **must** allow a user to have different permissions dependant on their currently assigned role.
...
3. The systme **must** allow a user to be a team leader for multiple projects. [^6]
...
### 10. Manager dashboard requirements (overlapping for both)
...
3.  A _Project_ **should** display whether it is sufficiently resourced, based on the sum of worker-hours required by its tasks and the number of workers assigned. [^17]
...
3. Posts **could** be viewable by all users by default. This option can be changed to be accessible only by certain levels (Manager, Team lead, User). [^20]
